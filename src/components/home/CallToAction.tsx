    ...
    <section className="relative py-24 overflow-hidden bg-white">
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center text-white">
        <motion.div
          ...
          viewport={{ once: true }}
        >
          <h2 className="text-2xl sm:text-3xl md:text-5xl lg:text-6xl font-display font-bold text-white leading-tight mb-6">
            Ready for Your Next <span className="text-white">Adventure?</span>
          </h2>
          <p className="text-base sm:text-lg md:text-2xl text-white leading-relaxed max-w-3xl mx-auto mb-8 px-4">
        ...
        <motion.div
          ...
          viewport={{ once: true }}
        >
          <div className="flex items-center space-x-3 text-white bg-white/10 backdrop-blur-sm rounded-xl px-6 py-3 border border-white/20">
            <Phone className="w-5 h-5 sm:w-6 sm:h-6 text-white" />
            <span className="text-white font-medium text-lg">Call us: +91 9773713859, +91 9724287467</span>
          </div>
          <div className="flex items-center space-x-3 text-white bg-white/10 backdrop-blur-sm rounded-xl px-6 py-3 border border-white/20">
            <Mail className="w-5 h-5 sm:w-6 sm:h-6 text-white" />
            <span className="text-white font-medium text-lg">Email: Info.flynzo@gmail.com</span>
          </div>
        </motion.div>

        {/* Special Offer */}
        <motion.div
          className="mt-16 inline-block bg-white/15 backdrop-blur-md rounded-2xl p-8 border border-white/30 shadow-2xl"
          initial={{ opacity: 0, scale: 0.9 }}
          whileInView={{ opacity: 1, scale: 1 }}
          transition={{ duration: 0.6, delay: 0.6 }}
          viewport={{ once: true }}
        >
          <div className="text-white font-bold text-2xl mb-3">Special Offer!</div>
          <div className="text-white text-lg mb-2">
            Book now and get <span className="font-bold text-yellow-300 text-xl">15% OFF</span> on your first trip
          </div>
          <div className="text-white/80 text-sm">*Terms and conditions apply</div>
        </motion.div>
      </div>
    </section>
  )
